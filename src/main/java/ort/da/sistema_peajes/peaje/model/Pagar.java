...
public class Pagar {
    ...
    public void realizarPagoSimple(Propietario propietario, Registro registro)
        ...
        if (!propietario.esBonificable()) {
            ...
            propietario.refrescarTransitoSinNotificar();
...
public class Pagar {
    ...
    boolean puedeTransitar = true;
    boolean esBonificable = false;

    // Verificar si el propietario puede transitar
    try {
        propietario.getEstadoPropietario().puedeTransitar();
    } catch (EstadoException e) {
        puedeTransitar = false;
    }

    // Verificar si es bonificable
    try {
        esBonificable = propietario.getEstadoPropietario().bonificable();
    } catch (Exception e) {
        esBonificable = false;
    }

    // Si está suspendido o deshabilitado, no registrar tránsito
    if (!puedeTransitar) {
        return;
    }

    // Registrar el tránsito siempre (penalizado y habilitado)
    completarRegistro(propietario, registro);

    // Solo aplicar bonificación y notificación si puede recibir bonificaciones
    if (esBonificable) {
        Puesto puesto = registro.getPuesto();
        Vehiculo vehiculo = registro.getVehiculo();
        int montoTarifa = registro.getMontoTarifa();
    ...
    private static void completarRegistro(Propietario propietario, Registro registro) throws SaldoException {
        ...
        propietario.getRegistros().add(registro);

        propietario.avisar(EventosSistema.TRANSITO_REALIZADO);
    }
    ...
    private static void descontarTransito(Propietario propietario, double monto) throws SaldoException {
        ...
        if (saldo < monto) {
            throw new SaldoException("Saldo Actual: " + saldo + "\n Saldo insuficiente para cobrar el total: " + monto);
        }
    ...
    private static void notificarTransito(Propietario propietario, String puesto, String matricula) {
        propietario.agregarNotificacion_PAGO_REALIZADO(
                Notificacion.notificarTransito(puesto, matricula)
        );
    }
}
